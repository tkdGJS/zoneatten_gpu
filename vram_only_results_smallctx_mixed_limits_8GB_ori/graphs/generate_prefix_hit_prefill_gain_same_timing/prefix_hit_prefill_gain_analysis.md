...
- A batch-level linear fit was estimated from `batch_total_computation_tokens -> batch_mean_prefill_ms`: `prefill ≈ 1.084816 * compute + -1847.677`
...
- `batch_prefix_hit_rate` vs `prefill time average / input token`: r = -0.548
- `batch_prefix_hit_rate` vs `estimated_prefix_hit_gain_ms`: r = 0.814
