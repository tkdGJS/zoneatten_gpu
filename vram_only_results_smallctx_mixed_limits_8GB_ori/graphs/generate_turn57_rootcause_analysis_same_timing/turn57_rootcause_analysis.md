...
- `turn 5` itself is not the real queueing onset. Combined mean blocking at turn 5 is `0.2 ms`, still effectively zero.
- The first structural queueing starts at `turn 6`: mean blocking rises to `114.8 ms` while mean resident KV per experiment reaches `3.523 GiB`.
- By `turn 7`, mean blocking reaches `268.6 ms`, mean prefill reaches `18430.6 ms`, and mean TTFT reaches `18699.2 ms`.
- Prefix reuse collapses across the same transition: global prefix-hit rate drops from `76.2%` at turn 5 to `78.2%` at turn 6 and `80.8%` at turn 7.
- Tail prefill also starts to break at `turn 6`: p99 prefill is `13878.8 ms` at turn 5, `20173.2 ms` at turn 6, and `25592.9 ms` at turn 7.
...
- Mean total resident KV before prefill is `2.780 GiB` at turn 5, `3.523 GiB` at turn 6, and `4.389 GiB` at turn 7.
...
- Mean total prompt KV after prefill is even larger: `2.820 GiB` at turn 5 and `4.452 GiB` at turn 7.

## Group-Level Difference At Turn 7

- Group2: mean input `3485.6`, mean prefix-hit `2786.0`, mean blocking `322.2 ms`, mean prefill `17155.4 ms`, p99 prefill `24711.6 ms`, mean resident KV per experiment `1.699 GiB`.
- Group1: mean input `5631.1`, mean prefix-hit `4576.0`, mean blocking `214.9 ms`, mean prefill `19705.9 ms`, p99 prefill `25592.9 ms`, mean resident KV per experiment `2.690 GiB`.
...
- Correlation, estimated resident KV per tenant vs TTFT: `0.685`
- Correlation, estimated resident KV per tenant vs prefill: `0.690`
- Correlation, input tokens vs TTFT: `0.682`
- Correlation, output tokens vs TTFT: `0.294`
