...
- In turns 5/6/7, correlation between computation tokens and prefill is `0.121`.
- In the same turns, correlation between resident KV per tenant and prefill is `0.243`.
- After subtracting the baseline `prefill = computation_tokens / 6`, correlation between resident KV and prefill residual is `0.237`.
- Correlation between computation tokens and that residual is `0.112`.
...
- Low KV: `resident_kv_mib <= 30.3`
- Mid KV: `30.3 < resident_kv_mib <= 96.7`
- High KV: `resident_kv_mib > 96.7`
