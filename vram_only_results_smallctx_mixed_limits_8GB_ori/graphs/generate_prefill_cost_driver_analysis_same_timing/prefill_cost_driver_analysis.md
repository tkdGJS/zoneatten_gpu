...
- Correlation between per-tenant resident KV and prefill cost per token is `-0.554` in turns 5/6/7.
...
- Across all turns, correlation between computation tokens and prefill is `0.357`.
- Across all turns, correlation between computation tokens and prefill residual is `0.351`.
- Across all turns, correlation between batch total resident KV and prefill residual is `0.860`.
- Across all turns, correlation between batch total resident KV and prefill cost per token is `0.204`.
...
- Correlation between batch total resident KV and prefill cost per token is `-0.044`.
- Correlation between batch total computation tokens and prefill cost per token is `-0.035`.
- Correlation between batch total resident KV and residual is `0.498`.
- Correlation between batch total computation tokens and residual is `0.502`.
...
- Low compute: `computation_tokens <= 727.7`
- Mid compute: `727.7 < computation_tokens <= 1107.0`
- High compute: `computation_tokens > 1107.0`
