...
- Correlation between per-tenant resident KV and prefill cost per token is `-0.352` in turns 5/6/7.
...
- Across all turns, correlation between computation tokens and prefill is `0.422`.
- Across all turns, correlation between computation tokens and prefill residual is `0.413`.
- Across all turns, correlation between batch total resident KV and prefill residual is `0.809`.
- Across all turns, correlation between batch total resident KV and prefill cost per token is `-0.145`.
...
- Correlation between batch total resident KV and prefill cost per token is `0.123`.
- Correlation between batch total computation tokens and prefill cost per token is `-0.084`.
- Correlation between batch total resident KV and residual is `0.424`.
- Correlation between batch total computation tokens and residual is `-0.210`.
...
- Low compute: `computation_tokens <= 1027.7`
- Mid compute: `1027.7 < computation_tokens <= 1044.0`
- High compute: `computation_tokens > 1044.0`
