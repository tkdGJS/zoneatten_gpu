...
- In turns 5/6/7, correlation between computation tokens and prefill is `0.137`.
- In the same turns, correlation between resident KV per tenant and prefill is `0.277`.
- After subtracting the baseline `prefill = computation_tokens / 6`, correlation between resident KV and prefill residual is `0.266`.
- Correlation between computation tokens and that residual is `0.123`.
...
- Low KV: `resident_kv_mib <= 38.8`
- Mid KV: `38.8 < resident_kv_mib <= 131.7`
- High KV: `resident_kv_mib > 131.7`
