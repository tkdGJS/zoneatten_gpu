...
- A batch-level linear fit was estimated from `batch_total_computation_tokens -> batch_mean_prefill_ms`: `prefill ≈ 0.266438 * compute + 24123.967`
...
- `batch_prefix_hit_rate` vs `prefill time average / input token`: r = 0.366
- `batch_prefix_hit_rate` vs `estimated_prefix_hit_gain_ms`: r = -0.121
- `batch_total_prefix_hit_tokens` vs `estimated_prefix_hit_gain_ms`: r = 0.410
