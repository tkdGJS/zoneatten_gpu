...
- A batch-level linear fit was estimated from `batch_total_computation_tokens -> batch_mean_prefill_ms`: `prefill ≈ 0.551728 * compute + 12665.615`
...
- `batch_prefix_hit_rate` vs `prefill time average / input token`: r = -0.406
- `batch_prefix_hit_rate` vs `estimated_prefix_hit_gain_ms`: r = 0.533
- `batch_total_prefix_hit_tokens` vs `estimated_prefix_hit_gain_ms`: r = 0.738
