...
- Correlation between per-tenant resident KV and prefill cost per token is `-0.507` in turns 5/6/7.
...
- Across all turns, correlation between computation tokens and prefill is `0.589`.
- Across all turns, correlation between computation tokens and prefill residual is `0.572`.
- Across all turns, correlation between batch total resident KV and prefill residual is `0.559`.
- Across all turns, correlation between batch total resident KV and prefill cost per token is `-0.285`.
...
- Correlation between batch total resident KV and prefill cost per token is `-0.248`.
- Correlation between batch total computation tokens and prefill cost per token is `-0.327`.
- Correlation between batch total resident KV and residual is `0.197`.
- Correlation between batch total computation tokens and residual is `0.128`.
...
- Low compute: `computation_tokens <= 710.0`
- Mid compute: `710.0 < computation_tokens <= 2207.0`
- High compute: `computation_tokens > 2207.0`
