...
- Correlation between per-tenant resident KV and prefill cost per token is `-0.484` in turns 5/6/7.
...
- Across all turns, correlation between computation tokens and prefill is `0.379`.
- Across all turns, correlation between computation tokens and prefill residual is `0.373`.
- Across all turns, correlation between batch total resident KV and prefill residual is `0.858`.
- Across all turns, correlation between batch total resident KV and prefill cost per token is `0.169`.
...
- Correlation between batch total resident KV and prefill cost per token is `0.009`.
- Correlation between batch total computation tokens and prefill cost per token is `0.044`.
- Correlation between batch total resident KV and residual is `0.424`.
- Correlation between batch total computation tokens and residual is `0.451`.
...
- Low compute: `computation_tokens <= 479.3`
- Mid compute: `479.3 < computation_tokens <= 1055.7`
- High compute: `computation_tokens > 1055.7`
