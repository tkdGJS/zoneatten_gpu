...
- A batch-level linear fit was estimated from `batch_total_computation_tokens -> batch_mean_prefill_ms`: `prefill ≈ 0.738404 * compute + 11394.541`
...
- `batch_prefix_hit_rate` vs `prefill time average / input token`: r = 0.101
- `batch_prefix_hit_rate` vs `estimated_prefix_hit_gain_ms`: r = -0.340
- `batch_total_prefix_hit_tokens` vs `estimated_prefix_hit_gain_ms`: r = 0.427
