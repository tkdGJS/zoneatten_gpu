...
- Group1 mean total TTFT: `2635.20 ms`
- Group2 mean total TTFT: `2502.02 ms`
- Group1 breakdown: blocking `0.03 ms`, prefill `2635.17 ms`, remaining `0.00 ms`
- Group2 breakdown: blocking `0.03 ms`, prefill `2501.99 ms`, remaining `0.00 ms`
- Prefill 차이가 가장 큰 구간은 `Group1` 쪽이며, 두 그룹 간 prefill 평균 차이는 `133.18 ms`입니다.
- 평균 total TTFT는 `Group1`가 더 크고, 차이는 `133.17 ms`입니다.

## Tenant 16

- Group1 mean total TTFT: `7418.86 ms`
- Group2 mean total TTFT: `7175.90 ms`
- Group1 breakdown: blocking `0.07 ms`, prefill `7418.78 ms`, remaining `0.00 ms`
- Group2 breakdown: blocking `0.09 ms`, prefill `7175.81 ms`, remaining `0.00 ms`
- Prefill 차이가 가장 큰 구간은 `Group1` 쪽이며, 두 그룹 간 prefill 평균 차이는 `242.98 ms`입니다.
- 평균 total TTFT는 `Group1`가 더 크고, 차이는 `242.96 ms`입니다.

## Tenant 32

- Group1 mean total TTFT: `9038.22 ms`
- Group2 mean total TTFT: `9099.00 ms`
- Group1 breakdown: blocking `196.61 ms`, prefill `8841.61 ms`, remaining `0.00 ms`
- Group2 breakdown: blocking `93.53 ms`, prefill `9005.47 ms`, remaining `0.00 ms`
- Prefill 차이가 가장 큰 구간은 `Group2` 쪽이며, 두 그룹 간 prefill 평균 차이는 `163.86 ms`입니다.
- 평균 total TTFT는 `Group2`가 더 크고, 차이는 `60.78 ms`입니다.
