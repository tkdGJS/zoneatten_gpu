...
- In turns 5/6/7, correlation between computation tokens and prefill is `0.149`.
- In the same turns, correlation between resident KV per tenant and prefill is `0.243`.
- After subtracting the baseline `prefill = computation_tokens / 6`, correlation between resident KV and prefill residual is `0.234`.
- Correlation between computation tokens and that residual is `0.138`.
...
- Low KV: `resident_kv_mib <= 37.1`
- Mid KV: `37.1 < resident_kv_mib <= 138.6`
- High KV: `resident_kv_mib > 138.6`
