...
- Correlation between per-tenant resident KV and prefill cost per token is `-0.476` in turns 5/6/7.
...
- Across all turns, correlation between computation tokens and prefill is `0.581`.
- Across all turns, correlation between computation tokens and prefill residual is `0.565`.
- Across all turns, correlation between batch total resident KV and prefill residual is `0.544`.
- Across all turns, correlation between batch total resident KV and prefill cost per token is `-0.260`.
...
- Correlation between batch total resident KV and prefill cost per token is `-0.095`.
- Correlation between batch total computation tokens and prefill cost per token is `-0.275`.
- Correlation between batch total resident KV and residual is `0.359`.
- Correlation between batch total computation tokens and residual is `0.143`.
...
- Low compute: `computation_tokens <= 710.7`
- Mid compute: `710.7 < computation_tokens <= 2208.0`
- High compute: `computation_tokens > 2208.0`
