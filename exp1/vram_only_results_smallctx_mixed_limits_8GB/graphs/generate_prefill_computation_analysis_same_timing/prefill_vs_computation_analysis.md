...
- In turns 5/6/7, correlation between computation tokens and prefill is `0.104`.
- In the same turns, correlation between resident KV per tenant and prefill is `0.097`.
- After subtracting the baseline `prefill = computation_tokens / 6`, correlation between resident KV and prefill residual is `0.088`.
- Correlation between computation tokens and that residual is `0.079`.
