...
- `turn 5` itself is not the real queueing onset. Combined mean blocking at turn 5 is `290.2 ms`, still effectively zero.
- The first structural queueing starts at `turn 6`: mean blocking rises to `5688.6 ms` while mean resident KV per experiment reaches `7.025 GiB`.
- By `turn 7`, mean blocking reaches `34710.8 ms`, mean prefill reaches `34394.0 ms`, and mean TTFT reaches `73906.9 ms`.
- Prefix reuse collapses across the same transition: global prefix-hit rate drops from `76.4%` at turn 5 to `79.0%` at turn 6 and `60.1%` at turn 7.
- Tail prefill also starts to break at `turn 6`: p99 prefill is `37765.8 ms` at turn 5, `67820.4 ms` at turn 6, and `63577.9 ms` at turn 7.
...
- Group2: mean input `4093.0`, mean prefix-hit `706.0`, mean blocking `41180.6 ms`, mean prefill `35227.0 ms`, p99 prefill `47800.9 ms`, mean resident KV per experiment `1.938 GiB`.
- Group1: mean input `13400.6`, mean prefix-hit `9811.0`, mean blocking `28241.0 ms`, mean prefill `33561.0 ms`, p99 prefill `63577.9 ms`, mean resident KV per experiment `6.484 GiB`.
...
- Correlation, estimated resident KV per tenant vs TTFT: `0.409`
- Correlation, estimated resident KV per tenant vs prefill: `0.551`
- Correlation, input tokens vs TTFT: `0.408`
- Correlation, output tokens vs TTFT: `0.046`
