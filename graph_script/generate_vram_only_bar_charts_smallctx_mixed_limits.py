...
def render_chart(
    ...
    for idx in range(tick_count + 1):
        ...
        draw.text((80, y - 8), format_tick(tick_value), font=FONT_TICK, fill=BLACK)
    ...
    limit_centers: Dict[str, List[float]] = defaultdict(list)
    current_tenant = None
    for row in rows:
        ...
        y1 = plot_bottom
        fill = ORANGE if row["history_limit_tokens"] == "2048" else BLUE
        draw.rectangle((x0, y0, x1, y1), fill=fill, outline=fill)
        center_x = (x0 + x1) / 2
        tenant_centers[tenant_id].append(center_x)
        limit_centers[row["history_limit_tokens"]].append(center_x)
    ...
    legend_gap = 30
    draw.rectangle(
        (legend_x0, legend_y0, legend_x0 + swatch_size, legend_y0 + swatch_size),
        fill=BLUE,
        outline=BLUE,
    )
    draw.text((legend_x0 + 30, legend_y0 - 1), GROUP1_LABEL, font=FONT_SMALL, fill=BLACK)
    draw.rectangle(
        (
            legend_x0,
            legend_y0 + legend_gap,
            legend_x0 + swatch_size,
            legend_y0 + legend_gap + swatch_size,
        ),
        fill=ORANGE,
        outline=ORANGE,
    )
    draw.text(
        (legend_x0 + 30, legend_y0 + legend_gap - 1),
        GROUP2_LABEL,
        font=FONT_SMALL,
        fill=BLACK,
    )
